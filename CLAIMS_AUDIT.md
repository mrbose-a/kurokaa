...
## 6. CAPABILITY GRID — audit + rewrite of 2026-08-21

The flat 12-item "What's inside" checklist was replaced with a structured capability grid: six categories under two meta-groups, **Run the restaurant** (Service Floor / Inventory / Team) and **Know the restaurant** (Intelligence / Cash & Books / Compliance). The hero's secondary CTA became **"See what Kurokaa runs"** → `#foundation`.

Every item was re-derived by reading the shipped code (`backend/api.py`, `backend/app/routers/`, `backend/app/kuro/`, `backend/app/services/`, `autoops-frontend/src/`) rather than from `CLAUDE.md` or the roadmap — the failure mode §0 already identified twice.

### 6.1 Findings that changed the copy

| Claim | Verdict | What shipped instead |
|---|---|---|
| **"GSTR-2B"** | **Mislabelled.** `GET /api/gst/gstr2b-export` reads the restaurant's own `gst_purchase_register` and emits 2B-shaped columns. Real GSTR-2B is an auto-drafted ITC statement generated by GSTN from *suppliers'* GSTR-1 filings — a taxpayer cannot produce their own. Useful to *reconcile against* 2B; not 2B. | "Purchase register & ITC" |
| **"files Indian tax returns"** | **Too strong.** Nothing in the codebase transmits to the GST portal. The exports are CSV/XLSX, and the in-product reports carry "VERIFY ALL VALUES BEFORE FILING" plus a note that a CA must validate the treatment. | "GST return preparation", plus an explicit note under the grid that Kurokaa does not file on the owner's behalf. Pricing row changed from "GST-ready billing & returns" → "GST billing & return preparation". |
| **"Food safety logs"** | **Narrower than "logs".** `OPENING_CHECKLIST_ITEMS` (4) + `CLOSING_CHECKLIST_ITEMS` (3) are a fixed, non-configurable per-business-day checklist with exception notes (`food_safety_checklists`, health checks FS.1–FS.3). Fridge temperature is ticked *"checked and within range"* — no temperature **value** is recorded anywhere. | "Opening & closing food-safety checks". Never "temperature logs" or "HACCP". |
| **Payroll PF/ESI/TDS** | **Sound as computation** (corroborates §0). Correct statutory rates and thresholds in `payroll_engine.py:592-600`. Caveats worth knowing: PF basic is a *configurable 50%-of-gross assumption* (`PF_BASIC_PERCENT`), TDS is a *flat-rate estimate*, and there is **no ECR or challan file output**. | "Payroll with PF, ESI & TDS" kept. Do not imply statutory filing or challan generation. |
| **"Demand forecasting"** | **Sound but modest.** `fc.dish_demand` is a 28-day day-of-week-matched trailing average of the restaurant's own sales (a Monday averages prior Mondays), falling back to the full D28 set on thin samples. No model, no ML. The source comment calls it "behavior for a pilot". | "Expected demand" — framed as history-derived, not predictive. Calibration is a pilot exercise (owner call). |

### 6.2 Confirmed absent — must not appear on the page

Re-verified this round; all four remain unshippable:

- **Offline-first POS** — no service worker in `autoops-frontend/public/`, no `services/db.ts`, no `POST /api/pos/transactions/batch`. Planned in CLAUDE.md Sprint 4; never built.
- **AI agent workflows** — the legacy orchestrator is still constructed at startup (`backend/api.py:126`), but Kuro replaced it and **nothing under `backend/app/kuro/` calls an LLM**. The feed is deterministic SQL over cached features.
- **AI inventory counts** — audits are manual wizard entry. No vision or photo-count path exists.
- **Thermal printer hardware** — the ESC/POS encoder (`services/print/escpos.ts`) is real and was verified in a Node harness, but pairing against physical hardware is still untested. Not listed as a grid item.

### 6.3 Capability found and added

**Bill shared on WhatsApp** — `POST /api/bills/generate` mints a token, `GET /api/bills/{token}` serves a public (unauthenticated) bill page, and four frontend surfaces open `wa.me` with the link. Previously only mentioned in the pricing list; now a Service Floor grid item.

### 6.4 Where AI is a true claim

Exactly three code paths call a model: menu extraction (`services/menu_import.py`), supplier-invoice parsing (`/api/purchases/parse-invoice`), and the superseded `app/agents/` loop. Everything Kuro surfaces is deterministic — which is the stronger claim to an owner deciding whether to trust a number, and is why the grid sells **traceability** ("every figure traces back to the sale, purchase or count it came from") rather than intelligence.

### 6.5 Deliberately not shown

Engineer-facing scale metrics (endpoint counts, router counts, table counts) were kept off the page. They measure implementation surface, not owner value.
